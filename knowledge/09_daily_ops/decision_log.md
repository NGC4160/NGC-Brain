...
| 2026-08-30 | Birdhouse is **not** shop process. Intended home is **NGC4160/personal-projects**. `gh repo create` was denied (`GraphQL: Resource not accessible by integration (createRepository)`), so `tools/birdhouse-print-shop/` stays in NGC-Brain until Ryan creates that private repo. | Ryan White | Do not git-filter. Do not dump into Origin unless GitHub create is available. |
