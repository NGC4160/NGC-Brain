#!/usr/bin/env python3
"""Google Drive OAuth helper — test connection and optionally save credentials."""
...
def run_authorize_flow(client_id: str, client_secret: str) -> str:
    ...
    flow = InstalledAppFlow.from_client_config(client_config, scopes=[SCOPE_DRIVE])
...
def main() -> int:
    ...
    parser.add_argument("--authorize", action="store_true", help="Local browser OAuth (Mac/desktop)")
    parser.add_argument(
        "--save-creds",
        nargs=3,
        metavar=("CLIENT_ID", "CLIENT_SECRET", "REFRESH_TOKEN"),
        help="Write the three Drive credentials into .env (non-interactive)",
    )
    parser.add_argument(
        "--query",
    ...
    if args.save_creds:
        client_id, client_secret, refresh_token = args.save_creds
        if not ENV_PATH.exists():
            example = ROOT / ".env.example"
            ENV_PATH.write_text(example.read_text() if example.exists() else "")
        save_env_value(ENV_PATH, "GOOGLE_DRIVE_CLIENT_ID", client_id)
        save_env_value(ENV_PATH, "GOOGLE_DRIVE_CLIENT_SECRET", client_secret)
        save_env_value(ENV_PATH, "GOOGLE_DRIVE_REFRESH_TOKEN", refresh_token)
        print(f"Saved Google Drive credentials to {ENV_PATH}")
